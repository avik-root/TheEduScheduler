...
export default async function YearSectionsPage({ params, searchParams }: { params: { departmentId: string, programId: string, yearId: string }, searchParams: { email?: string } }) {
  ...
  const redirectPath = `/admin/dashboard/departments/${department.id}/programs/${program.id}?email=${loggedInAdminEmail}`;
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                        <Link href={`/admin/dashboard/departments/${department.id}/programs/${program.id}?email=${loggedInAdminEmail}`}>
                    ...
                    <div className="flex items-center gap-2">
                        <AddSectionDialog departmentId={department.id} programId={program.id} yearId={year.id} adminEmail={primaryAdminEmail} />
                        <EditYearDialog departmentId={department.id} programId={program.id} year={year} adminEmail={primaryAdminEmail} variant="button" />
                        <DeleteYearDialog departmentId={department.id} programId={program.id} year={year} adminEmail={primaryAdminEmail} variant="button" onSuccessRedirect={redirectPath} />
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <SectionsList departmentId={department.id} programId={program.id} yearId={year.id} sections={year.sections || []} adminEmail={primaryAdminEmail} />
                </CardContent>
            </Card>
        </div>
      </main>
