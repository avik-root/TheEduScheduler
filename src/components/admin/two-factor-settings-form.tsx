...
import { setAdminTwoFactor } from '@/lib/admin';
import { Switch } from '@/components/ui/switch';
import { useRouter } from 'next/navigation';
import { Admin2FASchema } from '@/lib/validators/auth';
import { Button } from '@/components/ui/button';

type FormData = z.infer<typeof Admin2FASchema>;
...
export function TwoFactorSettingsForm({ admin, onSuccess }: TwoFactorSettingsFormProps) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      email: admin.email,
      isEnabled: admin.isTwoFactorEnabled || false,
      pin: '',
      currentPassword: '',
    },
  });
  ...
  async function onSubmit(data: FormData) {
    setIsLoading(true);
    const result = await setAdminTwoFactor(data);
