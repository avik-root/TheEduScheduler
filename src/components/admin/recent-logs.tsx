...
import { ClipboardList, User, Mail, Clock } from 'lucide-react';
...
export function RecentLogs({ logs }: RecentLogsProps) {
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><ClipboardList /> Recent Faculty Logins</CardTitle>
                <CardDescription>
                    A list of the most recent faculty login events.
                </CardDescription>
            </CardHeader>
            <CardContent>
                {logs.length === 0 ? (
